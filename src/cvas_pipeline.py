...
def _empty_model(
    *,
    ...
    normalized_function_io = normalize_function_io(function_io, [])
    model = {
        **_schema_header(),
        "blocks": [],
        "operations": [],
        "signals": [],
        "flow": serialize_flow(_empty_flow(normalized_function_io)),
...
def _call_provenance_from_metadata(metadata: Dict[str, object]) -> Dict[str, object]:
    parser = str(metadata.get("parser") or "unknown")
    if parser in {"ast", "clang"}:
        confidence = "high"
    elif parser == "text":
        confidence = "low"
    else:
    ...
    if isinstance(limitations, list) and limitations:
        provenance["limitations"] = [str(item) for item in limitations]
    return provenance
...
def build_call_instances(
    ...
    for _, caller_name, _, _, source_file in functions:
        ...
        call_metadata = analysis_results[caller_name].call_metadata
        provenance = _call_provenance_from_metadata(call_metadata)
        ordinal = 0
...
def _attach_analysis_metadata(
    ...
    *,
    ...
    """Attach stable analysis contract metadata to every model shape."""
    model["analysis_mode"] = analysis_options.mode
    model["analysis_backend"] = analysis_options.backend
...
def build_model(
    ...
    if not found:
        ...
        return _attach_analysis_metadata(
            _empty_model(
