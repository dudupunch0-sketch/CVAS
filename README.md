...
`--clang-arg`와 `--clang-compile-db` 이름은 legacy 호환성을 위해 유지됩니다. 현재 `full` 모드에서는 include/define/undef/system-include/std/language 정보를 재구성해 GCC dump pass에 전달하는 용도로도 사용됩니다. 향후 `--compile-arg`, `--compile-db` 같은 중립 alias로 migration할 수 있습니다.
...
`analysis_mode`, `analysis_backend`, `project_mode`, `duplicate_functions`는 정상 모델 생성 경로에서 포함됩니다. `gcc_dump`는 `--analysis-mode full`에서만 포함되는 선택 metadata이며, `status`는 `ok`, `failed`, `unavailable` 중 하나입니다. CVAS region이 없거나 region 안에서 함수 정의를 찾지 못하는 early-return 출력은 현재 최소 필드(`blocks`, `operations`, `signals`, `flow`, `diagram_hint`, `note`)만 포함할 수 있습니다.
